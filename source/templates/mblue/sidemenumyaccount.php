...
<h3>Pending Invoice Details</h3>
<table cellspacing="5">
<?php $arrsystemVendor = array("common","caltech","caltechravi","bhuvan","bhuvansankar","bhuvanarun","bhuvanvenkat","bhuvanmaha"); ?>

<?php 

     $extraSql = " invoice_id IN (SELECT id FROM crs_invoice_list WHERE archive_status = 0 AND payment_status = 'pending') GROUP BY invoice_vendor";
	$pendingInvoiceList = $objPage->getRecordsByCustomQuery("rental_invoice_vendor_wise",$field="invoice_vendor, SUM(invoice_amount) as totamount",$extraSql);

?>



<?php for($i=0;$i<count($pendingInvoiceList);$i++){ ?>
<tr><td><strong><?php echo $pendingInvoiceList[$i]['invoice_vendor'];?></strong></td><td>:</td><td><?php echo $pendingInvoiceList[$i]['totamount'];?></td></tr>
<?php }?>
</table>
