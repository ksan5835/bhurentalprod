...
class possale {
	...
	public function updateInvoiceAmountByVendorWise($arrData)
	{
		
		//print_r($arrData);
		$invoiceID = $arrData['curinvoiceid'];
		...
		for($i=0;$i<count($arrsystemVendor);$i++){
			...
			$vendorName = $arrsystemVendor[$i];
			$query = mysql_query("SELECT * from crs_rental_invoice_vendor_wise where invoice_id='".$invoiceID."' and invoice_vendor='".$vendorName."'");			
			$invoiceDetails = @mysql_fetch_array($query);
			$invoiceAmount = $arrData['amt_'.$vendorName];
			
			if($invoiceDetails){
				
				$arrNewData['id'] = $invoiceDetails['id'];				
				$arrNewData['invoice_amount'] = $invoiceAmount ;							
				$this->insertUpdateVendorWiseInvoiceEntry($arrNewData);	
				
			}else{			
				if($invoiceAmount > 0){
					$arrNewData['id'] = 0;
					$arrNewData['invoice_id'] = $invoiceID ;
					$arrNewData['invoice_amount'] = $invoiceAmount ;
					$arrNewData['invoice_vendor'] = $vendorName;				
					$this->insertUpdateVendorWiseInvoiceEntry($arrNewData);	
				}	
	...
	public function insertUpdateVendorWiseInvoiceEntry($arrData)
	{		
		$db = new dbquery();
		...
		if($arrData['id'] != '0'){		
			$db->update_table($arrData,TBL_PREFIX.'rental_invoice_vendor_wise',$arrData['id']);
		}else{			
			$db->insert_table($arrData,TBL_PREFIX.'rental_invoice_vendor_wise');
		}
	}	
