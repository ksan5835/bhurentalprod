...
<br />
<div class="ctlmenu">
<form name="frmsplitinvoice" method="post" action="index.php?option=com_pos&task=vendorinvoiceupdate">
<table>
...
<?php 
	...
	$query = mysql_query("SELECT * from crs_rental_invoice_vendor_wise where invoice_id='".$invID."' and invoice_vendor='".$vendorName."'");			
	$invoiceDetails = @mysql_fetch_array($query);
	$invoiceAmount = $invoiceDetails['invoice_amount'];

?>
<tr>
<td><?php echo $arrsystemVendor[$i];?></td>
<td><input type="text" name="amt_<?php echo $arrsystemVendor[$i];?>" value="<?php echo $invoiceAmount;?>" /></td>
</tr>
<?php } ?>
...
<input type="hidden" name="curinvoiceid" value="<?php echo $invID;?>" />
