...
class com_pos_Controller {
	...
	public function invoke($view,$task)
	{
		...
		{			
			...
			break;
			case "vendorinvoiceupdate":
				$this->invoiceupdatebyvendorwise($_POST);
			break;
	...
	function invoiceupdatebyvendorwise($arrData)
	{
	    $objPage = new possale();
		$objPage->updateInvoiceAmountByVendorWise($arrData);
		
		$cid = $arrData['cid'];
		$vid = $arrData['curinvoiceid'];		
		header("location: index.php?option=com_pos&view=invoice&cid=".$cid."&vid=".$vid);
		die;
	}
